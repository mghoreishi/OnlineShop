...
namespace _01_OnlineShopQuery.Query
{
    public class ProductCategoryQuery : IProductCategoryQuery
    {
        ...
        public ProductCategoryQueryModel GetProductCategoryWithProducstsBy(string slug, int currentPage, int pageSize)
        {
            var inventory = _inventoryContext.Inventory.Select(x =>
                new { x.ProductId, x.UnitPrice }).ToList();
            var discounts = _discountContext.CustomerDiscounts
            ...
            catetory.Products = catetory.Products.OrderBy(d => d.PictureTitle).Skip((currentPage - 1) * pageSize).Take(pageSize).ToList();
        ...
        public int GetCountProductCategoryWithProducstsBy(string slug)
        {
