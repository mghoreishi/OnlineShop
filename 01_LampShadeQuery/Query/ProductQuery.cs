...
namespace _01_OnlineShopQuery.Query
{
    public class ProductQuery : IProductQuery
    {
        ...
        public ProductQuerySearchModel Search(string value, int currentPage, int pageSize)
        {
            ...
            ProductQuerySearchModel productSearchModel = new()
            {
                Products = query.OrderByDescending(x => x.Id).Skip((currentPage - 1) * pageSize).Take(pageSize).ToList(),
                TotalCount = query.ToList().Count
            };



            foreach (var product in productSearchModel.Products)
            {
                var productInventory = inventory.FirstOrDefault(x => x.ProductId == product.Id);
                if (productInventory != null)
                {
